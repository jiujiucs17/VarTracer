...
print("Start defining convolutional neural network...")
...
traced_nnet = torch.jit.trace(nnet, example_input)
print("Model tracing completed")

print("Start defining loss function and optimizer...")
...
for epoch in range(1):
    ...
    for i, data in enumerate(trainloader, 0):
        ...
        optimizer.zero_grad()
        # Use traced_nnet for forward pass
        outputs = traced_nnet(inputs)
...
vt.exec_stack_json(output_path=exec_stack_json_output_path, output_name="exec_stack_task_1_1.json", show_progress=True)

print("Execution stack JSON generated")
print("Generating dependency tree JSON...")
dependency_output_path = f"{output_path}/dependency"
vt.dep_tree_json(output_path=dependency_output_path, output_name="dep_tree_task_1_1.json")
